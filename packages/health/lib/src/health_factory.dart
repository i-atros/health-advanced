...
class HealthFactory {
  ...
  Future<List<HealthDataPoint>> _computeAndroidBMI(DateTime startDate, DateTime endDate) async {
    ...
    for (var i = 0; i < weights.length; i++) {
      double? weight = weights[i].value?.toDouble();
      if (h != null && weight != null) {
        final bmiValue = weight / (h * h);
        final x = HealthDataPoint._(bmiValue, dataType, unit, weights[i].dateFrom, weights[i].dateTo, _platformType, _deviceId!, '', '');
  ...
  Future<List<HealthDataPoint>> _dataQuery(DateTime startDate, DateTime endDate, HealthDataType dataType) async {
    ...
    if (fetchedDataPoints != null) {
      return fetchedDataPoints.map<HealthDataPoint>((e) {
        ...
        ECGData? ecgData;
        if (e["ecg"] != null) {
          final ecgMap = Map<String, dynamic>.from(e["ecg"]);
          ecgData = ecgMap.isNotEmpty ? ECGData.fromJson(ecgMap) : null;
        }
